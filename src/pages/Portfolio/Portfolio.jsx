...
const Portfolio = () => {
    ...
    const getData = (page = 1) => {
        setLoading(true);
        axios.post('http://localhost:5004/api/plexus/portfolio/read/admin', { page })
            .then((res) => {
        ...
        onSubmit: async (values, { setSubmitting, resetForm }) => {
            ...
            try {
                ...
                const request = id !== undefined
                    ? axios.patch(`http://localhost:5004/api/plexus/portfolio/update/${id}`, requestData, config)
                    : axios.post('http://localhost:5004/api/plexus/portfolio/create', requestData, config);
    ...
    const handleDelete = async (portfolioId) => {
        if (!isSubmitting && window.confirm("Are you sure you want to delete this portfolio?")) {
            try {
                setIsSubmitting(true);
                const res = await axios.delete(`http://localhost:5004/api/plexus/portfolio/delete/${portfolioId}`);
                getData(currentPage);
                toast.success(res.data.message);
            } catch (err) {
                console.error(err);
