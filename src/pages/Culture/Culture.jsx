...
const Culture = () => {
    ...
    const getData = (page = 1) => {
        setLoading(true);
        axios.post('http://localhost:5004/api/plexus/culture/read', { page })
            .then((res) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const request = id !== undefined
                ? axios.patch(`http://localhost:5004/api/plexus/culture/update/${id}`, formData, config)
                : axios.post('http://localhost:5004/api/plexus/culture/create', formData, config);
    ...
    const handleDelete = async (itemId) => {
        if (!isSubmitting && window.confirm("Are you sure you want to delete this Culture Details?")) {
            try {
                setIsSubmitting(true);
                const res = await axios.delete(`http://localhost:5004/api/plexus/culture/delete/${itemId}`);
                getData(currentPage);
                toast.success(res.data.message);
            } catch (err) {
                console.error(err);
